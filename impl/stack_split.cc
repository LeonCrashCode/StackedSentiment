...
struct LSTMClassifier {
    vector<LookupParameter> p_word(DOMAIN_N);
    ...
    vector<LSTMBuilder*> l2rbuilder(DOMAIN_N);
    vector<LSTMBuilder*> r2lbuilder(DOMAIN_N);
	...
	p_R = model.add_parameters({TAG_HIDDEN_DIM, HIDDEN_DIM});
        p_bias_t = model.add_parameters({TAG_HIDDEN_DIM});

        p_feat2label = model.add_parameters({2, TAG_HIDDEN_DIM});
...
if(DEBUG)	cerr<<"bilstm done\n";
	...
	Expression cross_features = concatenate(args);
	Expression cross_i_r = tanh(i_bias_t + i_R * cross_features);
